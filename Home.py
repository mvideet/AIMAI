...
df = pd.read_csv("combo11.csv",nrows=100)
lbl = LabelEncoder()
df["label"] = lbl.fit_transform(df["label"])
input_text = st.text_area("Enter the clinical observations of the patient:")
num = st.slider("Enter how many possi"
                ""
                "ble protocols you would like to see:", min_value=1, max_value=8, step=1)
...
if st.button("Submit"):
    ...
    input_embedded=input_embedded[:1529]
    ...
    for i in range(num):
        for i in top_n_probs_idx:
            ...
            st.subheader("There is a " + "{:.2f}%".format(
                prob * 100) + " chance that the appropriate imaging protocol is " + class_name)
